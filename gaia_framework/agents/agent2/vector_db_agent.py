...
class VectorDatabase:
    ...
    def add_embeddings(
        ...
        try:
            log_dataobject_step(
                data_object, "Input Text to embedding indexing", log_file
            )
            logger.info("Adding embeddings to the vector database.")

            if isinstance(embeddings, (tuple, list)):
                embeddings = np.array(
                    [np.array(embedding).astype(np.float32) for embedding in embeddings]
                )
            ...
            if embeddings.shape[1] != self.dimension:
                raise ValueError(
                    f"Embedding dimension does not match the index dimension. Expected {self.dimension}, got {embeddings.shape[1]}."
                )
            ...
            log_dataobject_step(
                data_object, "After Embeddings Added and Indexed", log_file
            )
    ...
    def save_local(
        self, path, data_object: DataObject, log_file: str = "data_processing_log.txt"
    ):
        """
    ...
    def load_local(
        ...
        try:
            ...
            log_dataobject_step(
                data_object, "After Embeddings Loaded Locally", log_file
            )
    ...
    def get_similarity_indices(
        ...
        try:
            log_dataobject_step(
                data_object, "Input Text to get similarity indices", log_file
            )
            ...
            log_dataobject_step(
                data_object, "After Similarity Indices Retrieved", log_file
            )
